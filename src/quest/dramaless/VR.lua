...
-- Whether this platform can do VR AT ALL. The Quest build supplies an
-- Android OpenXR loader and EGL/OpenGL ES transport beside this module.
...
function VR.supported()
  ...
  return os == "Windows" or os == "Android"
end

function VR.enabled()
  -- The standalone Quest shell is already immersive. Hand control to the
  -- gameplay session immediately instead of leaving the native launcher
  -- visible because an older per-save setting still says OFF.
  if rawget(_G, "QUEST_PANEL_ACTIVE") then return VR.supported() end
